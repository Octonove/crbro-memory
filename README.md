...
| **Cost** (what CRBRO adds to a session) | ~**750 tokens** of protocol block · **under 5k tokens** for the whole boot payload on a 1,145-neuron brain · **~6.5k tokens** of tool definitions · **<1 ms** local recall over 300 facts | The boot block is paid once. The 750 figure is the protocol text alone; what boot RETURNS also carries hot topics, the active context and recent sessions, and on a mature brain that reached 20,352 tokens until 2.0.3 put a declared ceiling on it — 4,989 measured on the same brain after. No read is allowed past that ceiling now, and anything shortened says so. The 15 tool definitions (26,074 characters of description + input schema, measured with a real `tools/list` on 2.0.2 and divided by 4; 30,819 counting the output schemas of the three readers, ~7.7k tokens) are paid on every request by clients that load all tools (Claude Desktop, Cursor); Claude Code defers them and pays only for the ones it uses. Fewer tools, not fewer characters: the 23 of 1.13 measured 21,662 (~5.4k tokens), because each parameter's text now lives in the tool that absorbed it |
